...
export async function initPostHog() {
  ...
  // api_host MUST come from env var — no fallback to us.i.posthog.com
  // NEXT_PUBLIC_ vars are inlined at build time, so Vercel must have this set before building
  if (!host) {
    console.warn("PostHog: NEXT_PUBLIC_POSTHOG_HOST not set, skipping init")
    return
  }
